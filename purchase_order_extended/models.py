...
class PurchaseOrder(models.Model):
	...
	_columns = {
			'manual_name': osv.fields.char('Manual Name', states=READONLY_STATES, related='name', store=False, help='Use this field to manually override the assigned name of the object.'),
			'state': osv.fields.selection(STATE_SELECTION, 'Status', readonly=True, help="The status of the purchase order or the quotation request. A quotation is a purchase order in a 'Draft' status. Then the order has to be confirmed by the user, the status switch to 'Confirmed'. Then the supplier must confirm the order to change the status to 'Approved'. When the purchase order is paid and received, the status becomes 'Done'. If a cancel action occurs in the invoice or in the reception of goods, the status becomes in exception.", select=True),
			...
			'approver': osv.fields.many2one('res.users', 'Approved by', readonly=True, copy=False),
			'contact': osv.fields.many2one('res.users', 'In Charge', readonly=False, copy=True, states=READONLY_STATES),
			'parent': osv.fields.many2one('purchase.order', 'Parent Object', readonly=False, copy=False),
	}
