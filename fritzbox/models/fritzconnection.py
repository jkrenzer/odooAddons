...
class FritzXmlParser(object):
	...
	result = None
        logstring_auth = 'without authentication'
        if self.user or self.password:
            result = requests.get(source, auth=HTTPDigestAuth(self.user,self.password), verify=False)
	    logstring_auth = 'with authentication credentials %s:%s' % (self.user,self.password)
        else:
            result = requests.get(source, verify=False)
        _logger.info('Trying to access URL %s %s. Response: %s' % (source, logstring_auth, result))
        self.root = etree.fromstring(result.content)
...
class FritzConnection(object):
    ...
    def __init__(self, address=FRITZ_IP_ADDRESS,
                       ...
                       protocol = 'https',
                       path=None,
                       descfiles=[FRITZ_IGD_DESC_FILE, FRITZ_TR64_DESC_FILE],
                       services_json=None):
        ...
        FritzXmlParser.password = password
        FritzXmlParser.protocol = protocol
        FritzXmlParser.path = path
    ...
    def _read_descriptions(self, descfiles=[]):
        """
        Read and evaluate the igddesc.xml file
        and the tr64desc.xml file if a password is given.
        """
        for descfile in descfiles:
            _logger.debug('Reading description file: %s' % descfile)
            parser = FritzDescParser(descfile)
    ...
    def _read_services(self, services):
        ...
        for service in services:
            _logger.debug('Reading SCDP for service: %s' % service.name)
            parser = FritzSCDPParser(service)
